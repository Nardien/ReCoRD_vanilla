...
class Writer(object):
    def __init__(self, args):
        ...
        self.logdir = os.path.join(args.output_dir, "logs")
        if os.path.exists(self.logdir):
            shutil.rmtree(self.logdir)
        self.writer = SummaryWriter(logdir=self.logdir)

        self.qa_loss = 0
        self.logging_qa_loss = 0
        
        self.logging_step = 20

    def report_loss(self, qa_loss, global_step):
        ...
        if global_step % self.logging_step != 0:
            return
        self.writer.add_scalar("Training QA loss",
            (self.qa_loss - self.logging_qa_loss) / self.logging_step, global_step)
        self.logging_qa_loss = self.qa_loss

    def report_acc(self, exact, f1, epoch):
        if self.local_rank not in (-1, 0):
            return

        self.writer.add_scalar("Dev Exact Match", exact, epoch)
        self.writer.add_scalar("Dev F1", f1, epoch)
...
class RecordProcessor(object):
    ...
    def get_test_examples(self, data_dir):
        with open(os.path.join(data_dir, self.test_file)) as reader:
            input_data = json.load(reader)["data"]
        return self._create_examples(input_data)
